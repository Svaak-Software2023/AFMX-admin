...
import { StyledUpdateButton, StyledViewButton, exportToExcel } from 'src/views/icons'
...
import { allRemark } from 'src/store/features/remarkSlice'
...
const TotalComplaints = () => {
  ...
  const nonExisting = complaintData.filter(item => item.complaineeId == null)

  const existingClient = complaintData.filter(item => {
    if (item.complaineeId) {
      return item
    }
  })

  const existing = complaintData.length - nonExisting.length

  //-------------------------------------Use Effects -----------------------
  useEffect(() => {
    const result = complaintData.filter(item => {
      const customerName = item.customerName.toLowerCase().includes(search.toLowerCase())

      return customerName
    })
    setFilter(result)
  }, [search, complaintData])
              ...
              >
                <Box className='actions-right' sx={{ mr: 2, display: 'flex', alignItems: 'center' }}>
                  ...
                  <Box sx={{ display: 'flex', marginLeft: '10px' }}>
                    ...
                    <Box>
                      <Typography sx={{ fontSize: '1.2rem', textAlign: 'left' }}>{complaintData.length}</Typography>

...
const ClosedComplaints = () => {
  ...
  const nonExisting = closedComplaints.filter(item => item.complaineeId == null)

  const existingClient = closedComplaints.filter(item => {
    if (item.complaineeId) {
      return item
    }
  })

  const existing = closedComplaints.length - nonExisting.length

  //-------------------------------------Use Effects -----------------------
  useEffect(() => {
    const result = closedComplaints.filter(item => {
      const customerName = item.customerName.toLowerCase().includes(search.toLowerCase())

      return customerName
    })
    setFilter(result)
  }, [search, closedComplaints])
              ...
              >
                <Box className='actions-right' sx={{ mr: 2, display: 'flex', alignItems: 'center' }}>
                  ...
                  <Box sx={{ display: 'flex', marginLeft: '10px' }}>
                    ...
                    <Box>
                      <Typography sx={{ fontSize: '1.2rem', textAlign: 'left' }}>{closedComplaints.length}</Typography>
                      <Typography sx={{ fontSize: '0.7rem' }}> Closed Complaints</Typography>
                    </Box>
                  </Box>

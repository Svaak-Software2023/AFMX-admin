...
} from 'src/store/features/productAndcategorySlice'
...
const Index = () => {
  ...
  const columns = [
...

    {
      ...
      selector: row =>
        row.isActive ? (
          <Button onClick={() => updateStatus(row.productCategoryId, false)}>
            <Active>Active</Active>
          </Button>
        ) : (
          <Button onClick={() => updateStatus(row.productCategoryId, true)}>
    ...
    },
    {
      ...
      cell: row => (
        <>
          <VisibilitySharpIcon
            onClick={() => router.push(`/categories/${row.productCategoryId}/products`)}
            titleAccess='view products'
  ...
  const handleChange = e => {
    ...
    setFormData({
      ...formData,
      [name]: value,
      isActive: name == 'isActive' ? value : formData.isActive
    })
  }
